  ...
  <style>
    ...
    * {
    ...
    .hero {
      ...
      color: var(--white);
      text-align: center;
      background: linear-gradient(115deg, #041638, #0c3a7b);
    }
    ...
    #pesquisaPlaca:focus {
      ...
      box-shadow: 0 0 0 3px rgba(37, 99, 199, 0.14);
    }
...
<body>
  ...
  <main class="container">
    <section class="card">
      ...
      <div class="search-area">
        <label class="search-label" for="pesquisaPlaca">Pesquisar placa do caminhão</label>
        <div class="search-box">
          ...
          >
          <button id="limparPesquisa" onclick="limparPesquisa()" aria-label="Limpar pesquisa">×</button>
        </div>
      </div>
      ...
      <div id="semResultados" class="sem-resultados">
        Nenhum caminhão encontrado para esta placa.
      </div>
    </section>
  </main>
  ...
  <script>
    ...
    function exibirDados(lista) {
      ...
      lista.forEach(item => {
        ...
        tr.innerHTML = `
          ...
          <td style="text-align: center;">
            <button class="btn-check" onclick="abrirChecklist(${indexOriginal})" title="Ver checklist completo">
              📋
            </button>
          </td>
        `;
      ...
      tabela.style.display = lista.length ? "table" : "none";
      semResultados.style.display = lista.length ? "none" : "block";
    }

    function filtrarPlacas() {
      const campo = document.getElementById("pesquisaPlaca");
      const termo = normalizarTexto(campo.value);
      const botaoLimpar = document.getElementById("limparPesquisa");

      botaoLimpar.style.display = campo.value ? "block" : "none";

      const filtrados = dados
        .map((row, index) => ({ row, index }))
        .filter(item => normalizarTexto(item.row[1]).includes(termo));
    ...
    function mostrarErro(mensagem) {
      ...
      loading.className = "erro-carregamento";
      loading.innerHTML = `
        ${mensagem}
        <br>
    ...
    async function carregarDados() {
      ...
      const tempoLimite = setTimeout(() => controller.abort(), 15000);
      ...
      try {
        ...
        if (rows.length < 2) {
          throw new Error("Nenhum registro foi encontrado na planilha.");
        }

        headers = rows[0] || [];
        dados = rows.slice(1).filter(row => row.some(celula => celula.trim() !== ""));

        if (!dados.length) {
          throw new Error("Nenhum caminhão foi encontrado na planilha.");
        }

        exibirDados(dados.map((row, index) => ({ row, index })));
        ...
        const mensagem = err.name === "AbortError"
          ? "A planilha demorou muito para responder."
          : "Não foi possível carregar os dados da planilha.";

        mostrarErro(`${mensagem}<br><small>Verifique se a planilha está publicada para a web.</small>`);
